...
$old_error_handler = set_error_handler("form_state_scriptcase_error_handler");

function form_state_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['form_state']['contr_erro']) && $_SESSION['scriptcase']['form_state']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_form_state->contr_form_state->Apl_com_erro = true;
            if ($inicial_form_state->contr_form_state->NM_ajax_flag)
            {
                $inicial_form_state->contr_form_state->NM_ajax_info['errList']['geral_form_state'][] = $msg;
                return;
            }
            form_state_scriptcase_error_display($msg, $err_no);
        }
...
function form_state_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['form_state'])  && '' != $_SESSION['scriptcase']['error_icon']['form_state'])  ? $_SESSION['scriptcase']['error_icon']['form_state']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['form_state']) && '' != $_SESSION['scriptcase']['error_close']['form_state']) ? $_SESSION['scriptcase']['error_close']['form_state'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['form_state']) && '' != $_SESSION['scriptcase']['error_span_title']['form_state']) ? $_SESSION['scriptcase']['error_span_title']['form_state'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['form_state']) && '' != $_SESSION['scriptcase']['error_icon_title']['form_state']) ? $_SESSION['scriptcase']['error_icon_title']['form_state'] : '';
?>
