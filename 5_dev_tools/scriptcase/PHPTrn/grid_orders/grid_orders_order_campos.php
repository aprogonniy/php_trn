<?php
   ...
   session_start();
    $Ord_Cmp = new grid_orders_Ord_cmp(); 
    $Ord_Cmp->Ord_cmp_init();
   
class grid_orders_Ord_cmp
{
function Ord_cmp_init()
{
  global $sc_init, $path_img, $path_btn, $tab_ger_campos, $tab_def_campos, $tab_converte, $tab_labels, $embbed, $tbar_pos, $_POST, $_GET;
   ...
   $tab_labels["orderid"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_orders']['labels']["orderid"])) ? $_SESSION['sc_session'][$sc_init]['grid_orders']['labels']["orderid"] : "" . $this->Nm_lang['lang_orders_fild_orderid'] . "";
   $tab_ger_campos['status'] = "on";
   $tab_def_campos['status'] = "status";
   $tab_converte["status"]   = "status";
   $tab_labels["status"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_orders']['labels']["status"])) ? $_SESSION['sc_session'][$sc_init]['grid_orders']['labels']["status"] : "" . $this->Nm_lang['lang_orders_fild_status'] . "";
   $tab_ger_campos['total'] = "on";
   $tab_def_campos['total'] = "total";
   $tab_converte["total"]   = "total";
   $tab_labels["total"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_orders']['labels']["total"])) ? $_SESSION['sc_session'][$sc_init]['grid_orders']['labels']["total"] : "" . $this->Nm_lang['lang_orders_fild_total'] . "";
   ...
   if (isset($_SESSION['sc_session'][$sc_init]['grid_orders']['php_cmp_sel']) && !empty($_SESSION['sc_session'][$sc_init]['grid_orders']['php_cmp_sel']))
   {
       foreach ($_SESSION['sc_session'][$sc_init]['grid_orders']['php_cmp_sel'] as $NM_cada_field => $NM_cada_opc)
       {
           if ($NM_cada_opc == "off")
           {
              $tab_ger_campos[$NM_cada_field] = "none";
   ...
   if (!isset($_SESSION['sc_session'][$sc_init]['grid_orders']['ordem_select']))
   {
       $_SESSION['sc_session'][$sc_init]['grid_orders']['ordem_select'] = array();
   }
...
function Sel_processa_out_sel($campos_sel)
{
   ...
   $_SESSION['sc_session'][$sc_init]['grid_orders']['ordem_select'] = array();
   $_SESSION['sc_session'][$sc_init]['grid_orders']['ordem_grid']   = "";
   $_SESSION['sc_session'][$sc_init]['grid_orders']['ordem_cmp']    = "";
   foreach ($campos_sel as $campo_sort)
   {
       ...
       {
           $_SESSION['sc_session'][$sc_init]['grid_orders']['ordem_select'][$campo] = $ordem;
       }
   }
?>
...
?>
  function scSubmitOrderCampos(sPos, sType) {
   ...
   $.ajax({
    type: "POST",
    url: "grid_orders_order_campos.php",
    data: {
...
<?php
   foreach ($tab_ger_campos as $NM_cada_field => $NM_cada_opc)
   {
       if ($NM_cada_opc != "none")
       {
           if (!isset($_SESSION['sc_session'][$sc_init]['grid_orders']['ordem_select'][$tab_def_campos[$NM_cada_field]]))
           {
?>
...
<?php
   foreach ($_SESSION['sc_session'][$sc_init]['grid_orders']['ordem_select'] as $NM_cada_field => $NM_cada_opc)
   {
