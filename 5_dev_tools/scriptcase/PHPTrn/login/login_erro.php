...
$old_error_handler = set_error_handler("login_scriptcase_error_handler");

function login_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_login->contr_login->Apl_com_erro = true;
            if ($inicial_login->contr_login->NM_ajax_flag)
            {
                $inicial_login->contr_login->NM_ajax_info['errList']['geral_login'][] = $msg;
                return;
            }
            login_scriptcase_error_display($msg, $err_no);
        }
...
function login_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['login'])  && '' != $_SESSION['scriptcase']['error_icon']['login'])  ? $_SESSION['scriptcase']['error_icon']['login']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['login']) && '' != $_SESSION['scriptcase']['error_close']['login']) ? $_SESSION['scriptcase']['error_close']['login'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['login']) && '' != $_SESSION['scriptcase']['error_span_title']['login']) ? $_SESSION['scriptcase']['error_span_title']['login'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['login']) && '' != $_SESSION['scriptcase']['error_icon_title']['login']) ? $_SESSION['scriptcase']['error_icon_title']['login'] : '';
?>
