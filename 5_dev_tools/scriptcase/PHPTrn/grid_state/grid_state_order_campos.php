<?php
   ...
   session_start();
    $Ord_Cmp = new grid_state_Ord_cmp(); 
    $Ord_Cmp->Ord_cmp_init();
   
class grid_state_Ord_cmp
{
function Ord_cmp_init()
{
  global $sc_init, $path_img, $path_btn, $tab_ger_campos, $tab_def_campos, $tab_converte, $tab_labels, $embbed, $tbar_pos, $_POST, $_GET;
   ...
   if ($use_alias == "S")
   {
       $tab_def_campos['state'] = "state";
       $tab_converte["state"]   = "state";
   }
   else
   {
       $tab_def_campos['state'] = "st.name";
       $tab_converte["st.name"]   = "state";
   }
   $tab_labels["state"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_state']['labels']["state"])) ? $_SESSION['sc_session'][$sc_init]['grid_state']['labels']["state"] : "" . $this->Nm_lang['lang_state_fild_name'] . "";
   $tab_ger_campos['st_stateid'] = "on";
   if ($use_alias == "S")
   {
       $tab_def_campos['st_stateid'] = "st_stateid";
       $tab_converte["st_stateid"]   = "st_stateid";
   }
   else
   {
       $tab_def_campos['st_stateid'] = "st.stateid";
       $tab_converte["st.stateid"]   = "st_stateid";
   }
   $tab_labels["st_stateid"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_state']['labels']["st_stateid"])) ? $_SESSION['sc_session'][$sc_init]['grid_state']['labels']["st_stateid"] : "" . $this->Nm_lang['lang_state_fild_stateid'] . "";
   ...
   if (isset($_SESSION['sc_session'][$sc_init]['grid_state']['php_cmp_sel']) && !empty($_SESSION['sc_session'][$sc_init]['grid_state']['php_cmp_sel']))
   {
       foreach ($_SESSION['sc_session'][$sc_init]['grid_state']['php_cmp_sel'] as $NM_cada_field => $NM_cada_opc)
       {
           if ($NM_cada_opc == "off")
           {
              $tab_ger_campos[$NM_cada_field] = "none";
   ...
   if (!isset($_SESSION['sc_session'][$sc_init]['grid_state']['ordem_select']))
   {
       $_SESSION['sc_session'][$sc_init]['grid_state']['ordem_select'] = array();
   }
...
function Sel_processa_out_sel($campos_sel)
{
   ...
   $_SESSION['sc_session'][$sc_init]['grid_state']['ordem_select'] = array();
   $_SESSION['sc_session'][$sc_init]['grid_state']['ordem_grid']   = "";
   $_SESSION['sc_session'][$sc_init]['grid_state']['ordem_cmp']    = "";
   foreach ($campos_sel as $campo_sort)
   {
       ...
       {
           $_SESSION['sc_session'][$sc_init]['grid_state']['ordem_select'][$campo] = $ordem;
       }
   }
?>
...
?>
  function scSubmitOrderCampos(sPos, sType) {
   ...
   $.ajax({
    type: "POST",
    url: "grid_state_order_campos.php",
    data: {
...
<?php
   foreach ($tab_ger_campos as $NM_cada_field => $NM_cada_opc)
   {
       if ($NM_cada_opc != "none")
       {
           if (!isset($_SESSION['sc_session'][$sc_init]['grid_state']['ordem_select'][$tab_def_campos[$NM_cada_field]]))
           {
?>
...
<?php
   foreach ($_SESSION['sc_session'][$sc_init]['grid_state']['ordem_select'] as $NM_cada_field => $NM_cada_opc)
   {
