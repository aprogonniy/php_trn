...
$old_error_handler = set_error_handler("register_scriptcase_error_handler");

function register_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['register']['contr_erro']) && $_SESSION['scriptcase']['register']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_register->contr_register->Apl_com_erro = true;
            if ($inicial_register->contr_register->NM_ajax_flag)
            {
                $inicial_register->contr_register->NM_ajax_info['errList']['geral_register'][] = $msg;
                return;
            }
            register_scriptcase_error_display($msg, $err_no);
        }
...
function register_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['register'])  && '' != $_SESSION['scriptcase']['error_icon']['register'])  ? $_SESSION['scriptcase']['error_icon']['register']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['register']) && '' != $_SESSION['scriptcase']['error_close']['register']) ? $_SESSION['scriptcase']['error_close']['register'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['register']) && '' != $_SESSION['scriptcase']['error_span_title']['register']) ? $_SESSION['scriptcase']['error_span_title']['register'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['register']) && '' != $_SESSION['scriptcase']['error_icon_title']['register']) ? $_SESSION['scriptcase']['error_icon_title']['register'] : '';
?>
