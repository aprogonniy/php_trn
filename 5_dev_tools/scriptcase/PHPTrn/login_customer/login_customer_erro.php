...
$old_error_handler = set_error_handler("login_customer_scriptcase_error_handler");

function login_customer_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['login_customer']['contr_erro']) && $_SESSION['scriptcase']['login_customer']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_login_customer->contr_login_customer->Apl_com_erro = true;
            if ($inicial_login_customer->contr_login_customer->NM_ajax_flag)
            {
                $inicial_login_customer->contr_login_customer->NM_ajax_info['errList']['geral_login_customer'][] = $msg;
                return;
            }
            login_customer_scriptcase_error_display($msg, $err_no);
        }
...
function login_customer_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['login_customer'])  && '' != $_SESSION['scriptcase']['error_icon']['login_customer'])  ? $_SESSION['scriptcase']['error_icon']['login_customer']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['login_customer']) && '' != $_SESSION['scriptcase']['error_close']['login_customer']) ? $_SESSION['scriptcase']['error_close']['login_customer'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['login_customer']) && '' != $_SESSION['scriptcase']['error_span_title']['login_customer']) ? $_SESSION['scriptcase']['error_span_title']['login_customer'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['login_customer']) && '' != $_SESSION['scriptcase']['error_icon_title']['login_customer']) ? $_SESSION['scriptcase']['error_icon_title']['login_customer'] : '';
?>
